function net = poly_init_binary_tree_convNet(varargin)
...
if numel(opts.hiddenLayers) == 1
    rf_size = opts.inputSize(1);
    stride = 1;
else
    rf_size = 2;
    stride = 2;
...
net.layers{end+1} = struct('type', 'conv', ...
                           'weights', {{init_weight(opts,rf_size,1,previous_featNum,current_featNum, 'single'), opts.init_scale*randn( 1, current_featNum, 'single')}}, ...
                           'learningRate', lr, ...
                           'stride', stride, ...
                           'pad', 0 ) ;
...
for count = 2:numel(opts.hiddenLayers)
    ...
    net.layers{end+1} = struct('type', 'conv', ...
                               'weights', {{init_weight(opts,2,1,previous_featNum,current_featNum, 'single'),  opts.init_scale*randn(1, current_featNum, 'single')}}, ...
                               'learningRate', lr, ...
                               'stride', 2, ...
                               'pad', 0 ) ;
    if opts.disable_bias
