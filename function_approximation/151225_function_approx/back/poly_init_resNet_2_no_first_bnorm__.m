function net = poly_init_resNet_2_no_first_bnorm(varargin)
...
addpath ~/Dropbox/loop-share/matlab/y2016spring/160110_convcb_cifar/
lr = [1 1];
...
net.layers{end+1} = struct('type', 'conv', ...
                           'weights', {{init_weight(opts, 1, 1, previous_featNum, current_featNum, 'single'), init_scale_b*randn(1, current_featNum, 'single')}}, ...
                           'learningRate', lr, ...
                           'stride', 1, ...
                           'pad', 0 ) ;
...
for count = 2:numel(opts.hiddenLayers) 
    ...
    importance = 0.01;
    net.layers{end+1} = struct('type', 'conv', ...
                               'weights', {{init_weight(opts, 1, 1, previous_featNum, current_featNum, 'single')*importance,  init_scale_b*randn(1, current_featNum, 'single')*importance }}, ...
                               'learningRate', lr*importance, ...
                               'stride', 1, ...
                               'pad', 0 ) ;
    if opts.disable_bias
        disp('disable_bias...');
...
current_featNum = 1;
net.layers{end+1} = struct('type', 'conv', ...
                           'weights', {{init_weight(opts, 1, 1, previous_featNum, current_featNum, 'single'), zeros(1, current_featNum, 'single')}}, ...
                           'learningRate', lr, ...
                           'stride', 1, ...
                           'pad', 0 ) ;
if opts.disable_bias
