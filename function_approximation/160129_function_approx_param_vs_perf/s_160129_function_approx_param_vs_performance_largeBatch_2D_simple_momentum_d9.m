...
for i = 1:numel(candidates)
    for r = 1
        numEpochs = 200; enable_residual = false; first_layer_func = @return_1st_arg; disable_bias = false; all_t=tic; lr = 0.001; nVar = 2; nOrder = 2; rprop_p = 0; hiddenLayers = [candidates(i)]; force_batchSize = 3000; useBatchNorm = true; poly_eval_func = @poly_eval_n2_sinx_plus_2y_plus_cos3x_plus_2y; nonlinearity = struct('type','relu');
        if candidates(i) > 32
            lr = lr/10;
            numEpochs = numEpochs*10;
        end
        expDir = ['~/vlfeat_exp/poly_param_vs_perf/momentum_d9/bs' num2str(force_batchSize) '/numHidLayer_' num2str(numel(hiddenLayers)) '/resLearn_' num2str(enable_residual) '_poly_lr_' num2str(lr) '_rprop_' num2str(rprop_p)  '_hidden_' num_array_to_string_separated_by_underscore(hiddenLayers) '_nVar_' num2str(nVar) '_nOrder_' num2str(nOrder) '_' func2str(poly_eval_func) '_batchSize_' num2str(force_batchSize) '_bn_' num2str(useBatchNorm) '_' nonlinearity.type '_NOBias_' num2str(disable_bias) '_1stLyer_' func2str(first_layer_func) '/' num2str(r) ];
...
for i = 1:numel(candidates)
    for r = 1
        numEpochs = 200; enable_residual = false; first_layer_func = @return_1st_arg; disable_bias = false; all_t=tic; lr = 0.001; nVar = 2; nOrder = 2; rprop_p = 0; hiddenLayers = [candidates(i) candidates(i)]; force_batchSize = 3000; useBatchNorm = true; poly_eval_func = @poly_eval_n2_sinx_plus_2y_plus_cos3x_plus_2y; nonlinearity = struct('type','relu');
        expDir = ['~/vlfeat_exp/poly_param_vs_perf/momentum_d9/bs' num2str(force_batchSize) '/numHidLayer_' num2str(numel(hiddenLayers)) '/resLearn_' num2str(enable_residual) '_poly_lr_' num2str(lr) '_rprop_' num2str(rprop_p)  '_hidden_' num_array_to_string_separated_by_underscore(hiddenLayers) '_nVar_' num2str(nVar) '_nOrder_' num2str(nOrder) '_' func2str(poly_eval_func) '_batchSize_' num2str(force_batchSize) '_bn_' num2str(useBatchNorm) '_' nonlinearity.type '_NOBias_' num2str(disable_bias) '_1stLyer_' func2str(first_layer_func) '/' num2str(r) ];
        working_file = fullfile(expDir,'lock.mat');
        result_file  = fullfile(expDir,'result.mat');
        if ~exist(working_file,'file') && ~exist(result_file,'file')
...
for i = 1:numel(candidates)
    for r = 1
        numEpochs = 200; enable_residual = false; first_layer_func = @return_1st_arg; disable_bias = false; all_t=tic; lr = 0.001; nVar = 2; nOrder = 2; rprop_p = 0; hiddenLayers = [candidates(i) candidates(i) candidates(i)]; force_batchSize = 3000; useBatchNorm = true; poly_eval_func = @poly_eval_n2_sinx_plus_2y_plus_cos3x_plus_2y; nonlinearity = struct('type','relu');
        expDir = ['~/vlfeat_exp/poly_param_vs_perf/momentum_d9/bs' num2str(force_batchSize) '/numHidLayer_' num2str(numel(hiddenLayers)) '/resLearn_' num2str(enable_residual) '_poly_lr_' num2str(lr) '_rprop_' num2str(rprop_p)  '_hidden_' num_array_to_string_separated_by_underscore(hiddenLayers) '_nVar_' num2str(nVar) '_nOrder_' num2str(nOrder) '_' func2str(poly_eval_func) '_batchSize_' num2str(force_batchSize) '_bn_' num2str(useBatchNorm) '_' nonlinearity.type '_NOBias_' num2str(disable_bias) '_1stLyer_' func2str(first_layer_func) '/' num2str(r) ];
        working_file = fullfile(expDir,'lock.mat');
        result_file  = fullfile(expDir,'result.mat');
        if ~exist(working_file,'file') && ~exist(result_file,'file')
