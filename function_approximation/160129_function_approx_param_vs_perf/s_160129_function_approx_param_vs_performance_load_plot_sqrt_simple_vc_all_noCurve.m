...
%function_name  = 'Degree 2 polynomial, all monomials, uniform weights'
...
legend({'1 hidden pts','2 hidden pts','3 hidden pts'}); %legend({'1 hidden pts','1 hidden fit','2 hidden pts','2 hidden fit','3 hidden pts','3 hidden fit'});
...
current_info = all_info_1;
for i = 1:numel(current_info)
    x(end+1) = log(sum(current_info{i}.hiddenLayers(:)));
    y(end+1) = sqrt( nanmin([current_info{i}.(val_or_train).objective]) );
    s(end+1) = 40;
    c(end+1) = 0;
end
...
%fx1 = polyval(p,x1);
fx2 = f2(x1);
% [fx2 x1] = nonparametric_fit_by_binning_overlap(x,y,0.4,0.01);
%plot(x1,fx2,'LineWidth',3,'Color',colr);
